...
public class pktTreeCreateConf {
	...
	protected InetAddress MulticastIPAddress;
 	...
 	protected int port;  	  	
  	...
  	public pktTreeCreateConf(InetAddress address_) {
  	  	MessageID = 9;	
  	  	address = address_.getAddress();
  	}
  	
  	public pktTreeCreateConf(byte SequenceNumber_, InetAddress SourceIPAddress_, InetAddress IPCoreAddress_, Signalling agent) {
  	...
  	public void SetpktTreeCreateConf(DataInputStream qI, Signalling agent){
  		
  		synchronized(agent.signalling_use){	
		try{
                    
                    //settings variables		  		  		
                    MessageID = qI.readByte();//9;
    ...
    public byte[] GetpktTreeCreateConf(){
