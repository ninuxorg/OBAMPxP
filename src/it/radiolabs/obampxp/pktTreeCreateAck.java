...
public class pktTreeCreateAck {
	...
	protected InetAddress MulticastIPAddress;
 	...
 	protected int port;  	  	
  	...
  	public pktTreeCreateAck(InetAddress address_) {
  	  	MessageID = 8;	
  	  	address = address_.getAddress();
  	}
  	
  	public pktTreeCreateAck(byte SequenceNumber_, InetAddress SourceIPAddress_, InetAddress IPCoreAddress_, Signalling agent) {
  	...
  	public void SetpktTreeCreateAck(DataInputStream qI, Signalling agent){
  		synchronized(agent.signalling_use){	  		
		try{  		
                    
            //settings variables		  		  		
            MessageID = qI.readByte();//8;
    ...
    public byte[] GetpktTreeCreateAck(){
