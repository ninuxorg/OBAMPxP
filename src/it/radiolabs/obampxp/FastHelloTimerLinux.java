...
public class FastHelloTimerLinux {
    
    Timer timer;
    Signalling agent;
    
	
    public FastHelloTimerLinux(Signalling agent_) {
		agent = agent_;
        timer = new Timer();
        timer.schedule(new FastHelloLinuxTask(), 
        		0,
    			...
    			route_printb1.mark(1);
    				...
    				while ((str_route_print1 = route_printb1.readLine()) !=null){
    					...
    					for(int h=0; h<agent.JoinedList.length; h++){
    						if(agent.JoinedList[h]!=null && agent.JoinedList[h].IPaddress.equals(addressIP)){
    							
    							String metrica = str_route_print1.substring(54, 61).trim();
