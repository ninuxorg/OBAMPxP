...
public class pktTreeCreateNack {
	...
	protected InetAddress MulticastIPAddress;
  	...
  	public pktTreeCreateNack(InetAddress address_) {
  	  	MessageID = 6;	
  	  	address = address_.getAddress();
  	}
  	
  	public pktTreeCreateNack(byte SequenceNumber_, InetAddress SourceIPAddress_, InetAddress IPCoreAddress_, Signalling agent) {
  	...
  	public void SetpktTreeCreateNack(DataInputStream qI, Signalling agent){
  		
  		synchronized(agent.signalling_use){	  
		try{
                    
                    //settings variables		  		  		
                    MessageID = qI.readByte();//6;
    ...
    public byte[] GetpktTreeCreateNack(){
